...
const LARAVEL_TIP_API = "http://127.0.0.1:8000/api/tip"; // NEW: API for security tips
...
// Fallback security tips (used if API fails)
const securityTips = [
...
// Security Tip Loading
async function loadTip() {
  try {
    const res = await fetch(LARAVEL_TIP_API);
    ...
    if (data.tip) {
      ...
      // Optional: Log the source for debugging
      ...
      // If no tip in response, use fallback
      loadFallbackTip();
    }
  } catch (error) {
...
// Fallback tip function
function loadFallbackTip() {
  const dayOfYear = Math.floor(
    (Date.now() - new Date(new Date().getFullYear(), 0, 0)) / 86400000
  );
  const tip = securityTips[dayOfYear % securityTips.length];
...
// Preload liked wallpaper with fallback to another liked or gradient
function preloadLikedWallpaper(url, id, triedIds = []) {
  ...
  img.onerror = () => {
    // Add current id to tried list
    triedIds.push(id);
    // Try another liked wallpaper that hasn't been tried
    const remaining = likedWallpapers.filter(w => !triedIds.includes(w.id));
    if (remaining.length > 0) {
      const next = remaining[Math.floor(Math.random() * remaining.length)];
      preloadLikedWallpaper(next.url, next.id, triedIds);
    } else {
      // All liked wallpapers failed, show gradient fallback
      setFallbackGradient();
    }
  };
  img.src = url;
